...
class PeptideWindow(tk.Toplevel): #tk.Tk):
  def __init__(self, wn_previous):
    ...
    ### left area ###
    #Load button
    self.btn_loadFile = tk.Button(self, text='Upload input file', font=self.font_button, width=20, command=self.upload_peptide_file)
    ...
    ### centre area ###
    #Fileter
    self.lbl_fileter = tk.Label(self,text='Filter peptides based on',width=20,font=self.font_title)  
    ...
    #Quan info label
    self.lbl_quanInfo = tk.Label(self,text='Quan info', width=20, font=self.font_title)  
    ...
    self.chc_ptrAccessions = tk.Checkbutton(self, text='Show Protein Accessions', width=32, anchor="w", variable=self.var_chc_ptrAccessions, onvalue=1, offvalue=0)
    self.chc_ptrAccessions.grid(row=2, column=5, padx=5, pady=5)
    self.chc_ptrAccessions.config( font = self.font_checkbox )
  ...
  def manage_the_upload(self):
    ...
    '''
    #old method
    self.df.drop(['Checked','PSM Ambiguity','Annotated Sequence','Modifications','Modifications (all possible sites)',
                  'Qvality PEP', 'Qvality q-value', 'SVM_Score', '# Protein Groups', '# Proteins', '# PSMs',
                  'Positions in Master Proteins', 'Modifications in Master Proteins (all Sites)',
                  'Modifications in Master Proteins', '# Missed Cleavages', 'Theo. MH+ [Da]', 'Sequence Length',
                  'Confidence (by Search Engine): Sequest HT', 'Charge (by Search Engine): Sequest HT',
                  'DeltaScore (by Search Engine): Sequest HT', 'DeltaCn (by Search Engine): Sequest HT',
                  'Search Engine Rank (by Search Engine): Sequest HT', 'Concatenated Rank (by Search Engine): Sequest HT',
                  'm/z [Da] (by Search Engine): Sequest HT', 'DeltaM [ppm] (by Search Engine): Sequest HT',
                  'Deltam/z [Da] (by Search Engine): Sequest HT', 'RT [min] (by Search Engine): Sequest HT',
                  'Percolator q-Value (by Search Engine): Sequest HT', 'Percolator PEP (by Search Engine): Sequest HT',
                  'Percolator SVMScore (by Search Engine): Sequest HT', 'XCorr (by Search Engine): Sequest HT',
                  'Top Apex RT [min]'], inplace=True, axis=1, errors='ignore')
    ...
    valid_columns = ['Confidence', 'Sequence', 'Marked as', 'Master Protein Accessions',
                     'Master Protein Descriptions', 'Quan Info'] + abundances_columns
    #in the df keep only the useful columns
    #add extra optional column
    ...
    #if there isn't 'Protein Accessions' then delete the relative checkbox
    ...
    #to be sure, any old lists and checkboxes are removed
    if( hasattr(self, 'chcs_quant') ):
      #destroy all chechbox widget
      for i in self.chcs_quant:
        i.destroy()
      #clear the lists
      self.chcs_quant.clear()
      self.var_chcs_quant.clear()

    #get quant as value and put in a list
    quanList = self.df['Quan Info'].dropna().unique()
    quanList = quanList.tolist()
    #get position of markedAs label
    info2 = self.lbl_quanInfo.grid_info()
    actual_quantInfo_row = info2["row"] + 1 #add one to not overwrite label
    actual_quantInfo_column = info2["column"]
